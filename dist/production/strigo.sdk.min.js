...
</svg>`;function Ze(){let e=document.createElement("div");e.className="strigo-loader",e.innerHTML=wt,document.body.appendChild(e)}function qe(e){let t=document.querySelector(".strigo-loader"),i=()=>{let a=setInterval(()=>{t.style.opacity||(t.style.opacity="1");let l=parseFloat(t.style.opacity);l>0?t.style.opacity=(l-.1).toString():(t.style.pointerEvents="none",clearInterval(a))},200)};e.addEventListener("load",i)}var ue;(g=>{function t(){if(Ue(),oe(),He(),le&&se()){g.SDKType="SUBSCRIBER";let d=new Event("strigo-opened");window.dispatchEvent(d)}else g.SDKType="HOST";S.info("init called")}g.init=t;function i(d){if(g.SDKType==="SUBSCRIBER")return;S.info("setup started");let{email:T,token:c,development:_=!1}=d,{webApiKey:x,subDomain:L}=De();if(!_&&(!T||!c||!x||!L)){S.error("Please provide setup data");return}Be({email:T,initSite:ze(),token:c,webApiKey:x,subDomain:L,development:_}),Ie(),xe({parentElement:he(),url:_?`http://localhost:${SDK_HOSTING_PORT}/styles/strigo.css`:Le}),Ze();let b=Te(),G=te({parentElement:b,url:Pe(J()),classNames:be,id:"exercises"});qe(G),te({parentElement:b,url:J().initSite.href,classNames:Ge,id:"original-site"}),Oe(["#exercises","#original-site"],{sizes:[25,75],minSize:Re()?Me:Ne}),ie({currentUrl:J().initSite.href,isPanelOpen:!0}),je(),We(),S.info("setup finished")}g.setup=i;function a(){S.info("shutdown called"),Fe(),ae(),we()}g.shutdown=a;function l(d){Ke({eventName:d}),S.debug("sendEvent called",{eventName:d})}g.sendEvent=l})(ue||={});window.StrigoSDK=ue;window.StrigoSDK.init();})();
